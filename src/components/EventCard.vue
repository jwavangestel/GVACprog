...
function onChangePauze(event, index) {
 ...
 function Ddelete(index) {
  console.log (data_Store.events.events[index].datum)
  data_Store.deleteProgramma(data_Store.events.events[index].datum, 
                            data_Store.events.events[index].route, 
                            data_Store.events.events[index].pauzeplaats ).catch(error => {
      this.router.push(
       {
         name: 'ErrorDisplay',
         params: { error: error }
...
<template>
  ...
  <div class="w3-col  w3-container s1 m1 l1 "> 
    ...
    <p v-if ="editstatus[index] == 'on'">
      <button v-on:click="Ddelete(index)">
